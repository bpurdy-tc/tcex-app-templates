...
# TieTestCase is only imported under TYPE_CHECKING in profile.py, pydantic stores it as an
# unresolved ForwardRef at model-definition time and raises on first validation.
# Importing PipelineTask here (after TieTestCase is fully defined) and rebuilding resolves
# the ref without creating a circular import. `model_rebuild` is pydantic v2's
# `update_forward_refs`; the namespace goes in `_types_namespace` rather than as a kwarg.
from tcex_testing.apps.tie.profile import PipelineTask  # noqa: E402, PLC0415

PipelineTask.model_rebuild(_types_namespace={'TieTestCase': TieTestCase})
