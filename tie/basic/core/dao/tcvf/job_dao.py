...
class JobRequestDAO(JsonDBDAO[M], Generic[M]):
    ...
    def get_jobs_over_limit(self, max_jobs: int) -> Iterable[JobRequestModel]:
        """Get jobs over the limit."""
        paths = self.db.get_paths(self.model, sort_by='index', sort_order='desc')
        paths = paths[max_jobs:]
        for path in paths:
            yield self.db.load_from_path(JobRequestModel, path)

    def get_jobs_to_clean(self, max_jobs: int) -> Iterable[JobRequestModel]:
        """Get jobs over the limit."""
        hits = 0
        first_found = False
        # this iterates over the jobs with the most recent job first
        for job in self.db.load_all(JobRequestModel, sort_by='index', sort_order='desc'):
    ...
    def get_completed_before(self, before: arrow.Arrow) -> Iterable[JobRequestModel]:
    ...
    def get_next_for_task(self, task: TaskPathPipeABC) -> JobRequestModel | None:
        """Get the next job for the pipeline."""
        scheduled = self.db.get_paths(JobRequestModel, sort_by='index', sort_order='asc') or []

        ad_hoc_job = None
        for job in scheduled:
            job = self.db.load_from_path(JobRequestModel, job)
            if (
    ...
    def get_last_run_job(self, request_id, pipeline: str | None = None) -> JobRequestModel | None:
    ...
    def get_most_recent_scheduled_job(
        self, pipeline: str | None = None, model: type[JobRequestModel] = JobRequestModel
    ) -> JobRequestModel | None:
