...
class DownloadFilesResource(EndpointBase):
    ...
    def on_get(self, req: FalconRequest, resp: FalconResponse, query_params: QueryParamModel):  # noqa: ARG002
        """Download database file."""
        inmemory_file = BytesIO()
        with tarfile.open('', 'w:bz2', fileobj=inmemory_file) as tar:
            for path in self.download_path.rglob('*'):
                tar.add(
                    path, arcname=self.download_path.name / path.relative_to(self.download_path)
                )
